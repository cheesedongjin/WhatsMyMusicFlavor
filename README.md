...
- **클러스터 기반 신선 추천**: 장르·무드·에너지·인기도 지표를 결합한 곡 벡터를 클러스터링해 익숙한 취향과 다른 군집의 곡을 균형 있게 노출합니다.
...
- 장르 원-핫, 무드 가중치, 에너지·밸런스, 템포, 인기도 및 조회수 지표를 묶어 곡을 수치 벡터로 표현합니다.
- 전체 곡 라이브러리에 대해 KMeans 또는 MiniBatchKMeans를 한 번 학습한 뒤 결과를 캐시하여 추천 생성 시 재학습 비용을 제거했습니다.
- 토너먼트 상위 곡이 속한 군집과 다른 클러스터에서 일정 비율 이상을 선별해 "fresh" 추천이 낯선 사운드를 탐색하도록 합니다.
- 이러한 발견은 "새로운 군집 탐색"과 같은 추천 사유로 사용자에게 안내됩니다.
